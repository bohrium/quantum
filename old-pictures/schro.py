...
--- SETUP
    ...
    electron density is close to constant so W(r) scales like r^3.  On the
    other hand, for r>>1, W(r) is very close to Z'-1.  Interpolating, we
    expect that the charge-per-volume near r scales inverse-cubically with
    the effective charge Z-W(r).  So:

      W'(r) = C * r^2 * (Z-W(r))^3 
    
    By separation of variables we find that for some a^2 we have:

    !!!   W(r) = Z - 1 / sqrt(a^2 + (2C/3) r^3)

    !!! To get the right small-r behavior we must have a^2 = 1/(Z-Z'+1)^2.  Overall
...
--- RADIAL COMPONENT
    ...
    The solution for (n,l) should have
      ...
      phi(r) ~ C' r^n exp(-(Z-Z'+1)/n)r)    for r >> 1 
'''

ds = 0.0001
s = np.arange(ds, 1.0, ds)
E_a = 0.0
rr = -np.log(s) 
va = + (E_a/np.log(s)**2 + vs)
vb = - (s/2 + s/np.log(s))
vc = - s**2/2
